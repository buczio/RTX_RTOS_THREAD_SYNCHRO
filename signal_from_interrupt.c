...
#include "MKL25Z4.h"
#include "cmsis_os.h"
#include "Board_LED.h"		//support for built-in LEDs
...
void ADC1_2_IRQHandler (void)			//interrupt just to quick signal set
{
	osSignalSet(T_adc_ID,0x01);
	ADC1->SR &= ~(1 << 1);              
}

void adc_Thread (void const *argument)	//thread waiting for signal from interrput
{
	for (;;) 
	{	
...
void led_Thread1 (void const *argument) 
{
	for (;;)
	{
		osSignalWait(0x01,osWaitForever);		//LED on while conversion
		LED_On(1);                          
		osSignalWait(0x01,osWaitForever);	
		LED_Off(1);
	}
